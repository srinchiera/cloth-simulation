...
void Material::draw(Geometry& geometry, const Uniforms& extraUniforms) {
  ...
  for (int i = 0, n = programDesc_->uniforms.size(); i < n; ++i) {
      ...
      if (u) {
        if (u->type == ud.type && u->size >= ud.size) {
          ...
          case GL_SAMPLER_2D_SHADOW:
            if (ud.size != 1)
              throw runtime_error("Texture array unsupported for now");
            else {
              shared_ptr<Texture> tex = *u->getTextures();
              ...
              tex->bind();
              u->apply(ud.location, textureUnit);
              ++textureUnit;
            }
            break;
          default:
            u->apply(ud.location, -1);
          }
        }
        else {
          stringstream s;
          s << "Uniform variable " << ud.name << ": supplied value and declared variable do not match in type and/or size."
            << "\nSupplied value: type = " << string(getGlConstantName(u->type)) << ", size = " << u->size
            << "\nDeclared in shader: type = " << getGlConstantName(u->type) << ", size = " << ud.size;
          throw runtime_error(s.str());
        }
        break;
      }
    ...
    if (j == 2) {
      throw runtime_error(string("Uniform variable ") + ud.name
